<% cached current_item.URLSegment %>
	...
	<% with $current_item %>
		...
		<% end_if %>
            ...
            <br/>
            <div class="fb-like" data-href="$BaseHref{$Up.URLSegment}/show/$URLSegment" data-send="false"
								...
								<% if $AkismetMarked == 0 %>
                                    <li class="comment_$EvenOdd">
                                        <header>
                                            <figure>
												<% if $ShowGravatar %>
                                                    <img src="$Gravatar" width="$Top.SiteConfig.GravatarSize"
                                                         height="$Top.SiteConfig.GravatarSize" alt="$Name"/>
												<% end_if %>
                                            </figure>
                                            <address><strong>$Title</strong><%t NewsHolderPage_show.BYWHO "by" %>
                                                <strong><% if URL %><a href="$URL">$Name</a><% else %>$Name<% end_if %>
                                                </strong></address>
                                            <time datetime="$Created"><%t NewsHolderPage_show.ONDATE "on" %> $Created.Format(d
                                                -m-Y)
                                            </time>
                                        </header>
                                        <section>
